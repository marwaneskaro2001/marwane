...
if ($help || !$ip) {
  print <<'EOL';
mk.pl --port=dst-port --size=pkt-size --time=secs
...
Defaults:
  ...
  * mk is continuous unless --time is specified
  * mk is sent at line speed unless --bandwidth or --delay is specified
...
socket(mk, PF_INET, SOCK_DGRAM, 17);
...
for (;time() <= $endtime;) {
  ...
  send(mk, pack("a$psize","mk"), 0, pack_sockaddr_in($pport, $iaddr));
